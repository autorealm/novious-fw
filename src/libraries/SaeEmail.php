...
class SaeEmail {
	...
	var $callback_url = '';
 // SMTP发送失败时的回调地址，回调方式为post，postdata格式：timestamp=时间戳&from=from地址&to=to地址（如有多个to，则以,分割）
	...
	/**
	 * Set Callback Url
	 * 设置了此参数，SMTP发送失败时的回调地址，回调方式为post，postdata格式：
	 * 	timestamp=时间戳&from=from地址&to=to地址（如有多个to，则以,分割）
	 *
	 * @access public
	 * @param string
	 * @return void
	...
	public function attach($filename, $file_path) {
		$ret = $this->_sae_mail->setAttach(array(
				$filename => file_get_contents($file_path) 
		));
		if (!$ret) {
	...
	public function send() {
		if (!$this->_sae_mail->send()) {
			log_message('error', "Email Send Fail. Wrong: " 
					. $this->_sae_mail->errno() . ' ' . $this->_sae_mail->errmsg());
			return FALSE;
		} else {
			return TRUE;
		}
